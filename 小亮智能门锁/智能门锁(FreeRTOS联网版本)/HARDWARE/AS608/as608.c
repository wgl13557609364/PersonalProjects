...
extern volatile uint32_t alarmcount; //开门次数
//串口发送一个字节
...
void Add_FR(void)
{
  ...
  while(1)
  {
    ...
    switch (processnum)
    {
			...
			case 4:
				OLED_Clear();  //清屏
				OLED_ShowString(16,2,"A+B-*",16);
				OLED_ShowCHinese(56,2,122); //保存
				OLED_ShowCHinese(88,2,123);
				...
				while(1)
				{
					...
					OLED_ShowString(16, 6, "YOU ID =",16);
					OLED_ShowNum(80, 6, ID_NUM, 2,16);
				}
